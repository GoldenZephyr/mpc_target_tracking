...
def plot_mean_timing_comparison(ell_stats, nd_obs_stats, nd_stats, env):
    x = [2, 4, 6]
    fig,ax = plt.subplots()
    ax.scatter([2]*len(ell_stats[env]), [s[0] for s in ell_stats[env]])
    ax.scatter([4]*len(nd_obs_stats[env]), [s[0] for s in nd_obs_stats[env]])
    ax.scatter([6]*len(nd_stats[env]), [s[0] for s in nd_stats[env]])
    ax.set_xticks(x)
    xlabels =['Static Ellipsoid Decomposition (ours)', 'Dynamic Ellipsoid Decomposition', 'Dynamic Voronoi']
...
def plot_max_timing_comparison(ell_stats, nd_obs_stats, nd_stats, env):
    x = [2, 4, 6]
    fig,ax = plt.subplots()
    ax.scatter([2]*len(ell_stats[env]), [s[2] for s in ell_stats[env]])
    ax.scatter([4]*len(nd_obs_stats[env]), [s[2] for s in nd_obs_stats[env]])
    ax.scatter([6]*len(nd_stats[env]), [s[2] for s in nd_stats[env]])
    ax.set_xticks(x)
    xlabels =['Static Ellipsoid Decomposition (ours)', 'Dynamic Ellipsoid Decomposition', 'Dynamic Voronoi']
    ax.set_xticklabels(xlabels, fontsize=18)
...
plot_mean_timing_comparison(ellipsoids_stats, no_decomp_obs_stats, no_decomp_stats, 'blocks')
plot_mean_timing_comparison(ellipsoids_stats, no_decomp_obs_stats, no_decomp_stats, 'forest_')

plot_max_timing_comparison(ellipsoids_stats, no_decomp_obs_stats, no_decomp_stats, 'blocks')
plot_max_timing_comparison(ellipsoids_stats, no_decomp_obs_stats, no_decomp_stats, 'forest_')
