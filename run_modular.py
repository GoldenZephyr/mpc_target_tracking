...
def step_tracker(tracker, assignment_type, targets, targets_responsible, mpc_guess, pre_assignments):
    ...
    if 'ILP' in assignment_type:
        ...
        tsp_sol = solve_tsp(remaining_target_positions, tracker.state[:2], 'tracker_%d_tsp_%d.tsp' % (tracker.index, tsp_save_uuid))
    ...
    tsp_planning_times.append(time.time() - t0)
...
if __name__ == '__main__':
    ...
    #np.random.seed(3)
    #domain = 'forest'
    fn = 'domain_%s_trackers_%d_targets_%d_gen_%d_%f.txt' % (domain, n_trackers, n_targets, generation, time.time())
    with open('data_out/' + fn, 'w') as fo:
        fo.write('%d\n' % seed)
    ...
    tracker_positions = sample_tracker_positions(M_list, center_list, n_trackers, 5)
    for tracker_index in range(len(trackers.agent_list)):
        trackers.agent_list[tracker_index].unicycle_state[:2] = tracker_positions[tracker_index]
    trackers.synchronize_state()
    ...
    tsp_time_avg = np.mean(tsp_planning_times)
    tsp_time_std = np.std(tsp_planning_times)
    mpc_time_avg = np.mean(mpc_solve_times)
    mpc_time_std = np.std(mpc_solve_times)

    with open('data_out/' + fn, 'a+') as fo:
        fo.write('%d\n' % ix)
        fo.write('%f,%f\n' % (tsp_time_avg, tsp_time_std))
        fo.write('%f,%f\n' % (mpc_time_avg, mpc_time_std))
